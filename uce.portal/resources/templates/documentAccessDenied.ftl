<head>
    ...
    <style>
        ...
        .document-access-denied-screen {
            ...
            align-items: center;
            background: linear-gradient(135deg, var(--prime), var(--secondary));
            padding: 2rem;
        }
...
<section class="document-access-denied-screen">
    <div class="document-access-denied-card">
        ...
        <#assign noAccessText = (languageResource??) ? languageResource.get("noAccess") : "Access denied">
        <#assign backHomeText = (languageResource??) ? languageResource.get("backToHome") : "Back to home">
        <#assign goBackText = (languageResource??) ? languageResource.get("goBack") : "Go back">
        <#assign contactAdmin = (languageResource?? && languageResource.get("contactAdminText")?has_content)
            ? languageResource.get("contactAdminText")
